...
export default function FulfilledDatePage() {
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/supplement-vault/generate-fulfilled", {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
      ...
      link.download = "SupplementVault_FulfilledDate_Report.xlsx";
      link.click();
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center p-8">
      ...
      <h1 className="text-3xl font-bold text-[#1e1e1e] mb-1">
        💊 SupplementVault.lk — Fulfilled Date
      </h1>
      ...
      <div className="w-full max-w-[1100px]">
        <p className="text-xs font-semibold text-[#555] uppercase tracking-wide mb-2">
          AE Trading File <span className="text-[#999] normal-case font-normal">(optional)</span>
        </p>
      </div>
      <div className="flex gap-5 w-full max-w-[1100px] mb-4">
        <div className="max-w-[350px] flex-1">
          <DropZone
            title="AE Trading Export"
            icon="🏪"
            description="Drop the AE Trading export file here (.xlsx, .xls, .csv)"
            files={aeFiles}
      ...
      <div className="flex gap-4 w-full max-w-[1100px] mb-4 flex-wrap">
        <div className="flex flex-col flex-1 min-w-[200px]">
          <label className="text-[13px] font-semibold text-[#555] mb-1">Start Date</label>
          <input
            type="date"
        ...
        <div className="flex flex-col flex-1 min-w-[200px]">
          <label className="text-[13px] font-semibold text-[#555] mb-1">End Date</label>
          <input
            type="date"
